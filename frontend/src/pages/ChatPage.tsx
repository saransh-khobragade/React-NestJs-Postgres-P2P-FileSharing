...
export function ChatPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      const cs = await chatService.listConversations(numericId);
      setConversations(cs);
      if (cs.length > 0) setActiveId(cs[0]?.id ?? null);
    })();
  }, [user]);
      ...
      <main className="flex-1 flex flex-col">
        <div className="flex-1 p-4 space-y-2 overflow-y-auto">
          {messages.map((m) => {
            ...
            const isMine = Number.isFinite(currentUserId) && m.senderId === currentUserId;
            return (
              <div key={m.id} className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
                <div className={`px-3 py-2 rounded-xl max-w-[70%] ${isMine ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}>{m.content}</div>
              </div>
            );
          })}
        </div>
